...
# Системний промпт
# ---------------------------------------------------------------------------
SYSTEM_BASE = """Ти — досвідчений експерт-рибалка та копірайтер українського інтернет-магазину.
Твоя задача — згенерувати SEO-оптимізовану картку товару для моделі та визначити ВСІ характеристики, як загальні, так і для кожного варіанту.
Відповідай ВИКЛЮЧНО валідним JSON без markdown-обгортки.

Формат відповіді:
{
  "description_html": "<p>...</p><p>...</p><ul><li>...</li></ul>",
  "common_params": { "Ключ": "Значення", ... },
  "variant_params": { "Назва варіанту з запиту": { "Ключ": "Значення", ... } },
...
- common_params — лише поля, спільні для ВСІХ варіантів моделі (матеріал, бренд, тип).
- variant_params — індивідуальні атрибути КОЖНОГО варіанту (довжина, тест, вага, діаметр, колір). Ключі мають ТОЧНО збігатись із назвами варіантів, переданими в запиті.
- Якщо точних даних немає — реалістичне припущення на основі бренду/класу (AI).
...
FAMILY_INSTRUCTIONS: dict[str, str] = {
    ...
    "line": (
        ...
        "Діаметр, тест, розривне навантаження, довжину — поклади в variant_params для кожного варіанту. "
...
def save(conn: sqlite3.Connection, parent_key: str, result: dict) -> None:
    ...
    for v_name, v_params in variant_params.items():
        if v_params:
            conn.execute(
                "UPDATE variants SET delta_params_json = ? WHERE parent_key = ? AND name_raw = ?",
                (json.dumps(v_params, ensure_ascii=False), parent_key, v_name)
            )
